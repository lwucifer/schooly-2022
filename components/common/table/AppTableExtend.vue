    ...
    <div class="app-table">
      <table :class="{ ...tableCls, ...extTableCls, ...gridCls }">
        ...
        <tbody v-if="!hasDefaultSlot && hasData && !loading" :class="bodyCls">
          <template v-for="(cat, i) in cats">
            <tr
              :key="i"
              :class="{'selected': selectedItems.includes(cat), 'expanded': expandedRows.includes(cat)}"
              @click="$scopedSlots.expandedRowRender && toggleExpandedRows(cat)"
            >
              <td
            ...
            <tr
              v-if="$scopedSlots.expandedRowRender && expandedRows.includes(cat)"
              :key="'expand' + i"
            >
              <slot name="expandedRowRender" :row="cat" />
            </tr>
          </template>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    toggleExpandedRows(record) {
      this.$emit("checkClick", record);
      if (!record) return;
      if (this.expandedRows.includes(record)) {
        this.expandedRows = this.expandedRows.filter((item) => item !== record);
      } else {
        this.expandedRows = [...this.expandedRows, record];
      }
    },
    check(checked, item) {
      if (checked) {
