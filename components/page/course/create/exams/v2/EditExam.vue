  ...
  <div>
    <sub-block-section class="sub-block-section">
      ...
      <template v-slot:content>
        ...
        <hr class="mt-4 mb-2" />
        <question-list
          v-if="get(dataTest, 'statistic.questions', 0) > 0"
          :dataPayload="dataTest"
...
import QuestionList from "~/components/page/course/create/exams/v2/QuestionList";
  ...
  methods: {
    ...
    selectCreateQuesOpt(item) {
      ...
      if (item == "FROM_QUESTION_BANK") {
        this.$router.push(
          `/elearning/manager/bank/exams/choose-from-bank?category=TEST&type=${this.dataTest.type}&parent_id=${this.dataTest.id}&source=edit-test`
        );
      } else if (item == "UPLOAD") {
        this.modal = true;
    ...
    checkOpenCloseTime() {
      return true; //tmp disable validation
    ...
    handleOkTestBank() {
      const elearningId = get(this, "$route.query.elearning_id", "");
      this.$router.push(
        `/elearning/manager/bank/exams/choose-from-exam?parent_id=${this.dataTest.id}&type=${this.dataTest.type}&active_section=exam&active_content=edit-form&source=edit-test`
      );
    },
    async getExeDetail() {
      try {
  ...
  watch: {
    ...
    dataTest(_newVal) {
      this.payload = {
        ...
        id: _newVal.id || "",
        elearning_id: "",
        subject_id: (_newVal.subject && _newVal.subject.id) || "",
        grade_id: (_newVal.grade && _newVal.grade.id) || "",
        required: true,
        title: _newVal.name || "",
        type: _newVal.type || "",
        pass_score:
          (_newVal.settings && _newVal.settings.passing_score) || 0,
        reworks: (_newVal.settings && _newVal.settings.works) || 1,
        duration:
          (_newVal.settings && _newVal.settings.duration) || 0,
        category: "TEST",
        open_time:
          (_newVal.settings && _newVal.settings.start_time) || "",
        opentime_enable:
          (_newVal.settings && _newVal.settings.starttime_enable) ||
          false,
        close_time:
          (_newVal.settings && _newVal.settings.end_time) || "",
        closetime_enable:
          (_newVal.settings && _newVal.settings.endtime_enable) ||
          false,
        coefficient:
          (_newVal.settings && _newVal.settings.coefficient) || 1,
        weight: (_newVal.settings && _newVal.settings.weight) || "",
        method: _newVal.method || "",
      }
    }
  },
};
