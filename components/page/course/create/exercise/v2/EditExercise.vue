  ...
  <div>
    ...
    >
      ...
      <template v-slot:content>
        ...
        <div v-else-if="!get(exercise, 'upload', []).length">
          <question-list
...
import QuestionList from "~/components/page/course/create/exercise/v2/QuestionList"
...
export default {
  ...
  computed: {
    ...mapGetters(EXERCISE_CREATING_STORE, {
  ...
  methods: {
    ...
    selectCreateQuesOpt(item) {
      ...
      if (item == "FROM_QUESTION_BANK") {
        this.$router.push(
          `/elearning/manager/bank/exams/choose-from-bank?category=TEST&type=${get(this, 'exercise.type', '')}&parent_id=${get(this, 'exercise.id', '')}&item_name=${get(this, 'exercise.name', '')}&source=edit-exercise`
        );
    ...
    submitUploadQuestion() {
      this.$router.push(
        `/elearning/manager/bank/exams/upload-exam?category=TEST&type=${get(this, 'exercise.type', '')}&parent_id=${get(this, 'exercise.id', '')}&active_section=exercise&content_title=${get(this, 'exercise.name', '')}`
      );
    },
    submitImportTestQuestion() {
      this.$router.push(`/elearning/manager/bank/exams/choose-from-exam?parent_id=${get(this, 'exercise.id', '')}&type=${get(this, 'exercise.type', '')}&source=edit-exercise`);
    },
