    ...
    <div class="chapper-item__top">
      <div class="chapper-item__name">
        <template v-if="editNameToggle">
          <input ref="name" v-model="editedName" class="chapper-item__name-input" type="text" />
          <button class="chapper-item__btn chapper-item__btn--save" @click="handleEditChaperName">
        ...
        <template v-else>
          ...
          <button type="button" class="chapper-item__btn-edit-name" @click="showEditName">
            <IconEdit class="d-block icon fill-opacity-1" />
          </button>
        </template>
      </div>

      <div class="chapper-item__top-right">
        <button type="button" class="chaper-item__btn-add" @click="$emit('create', chapter)">
          <IconAddCircleOutline class="icon fill-opacity-1" />Thêm bài học
        </button>
        <button
          type="button"
          class="chapper-item__btn-collapse"
          :class="{ collapsed: collapsed }"
          @click="collapsed = !collapsed"
        >
          <IconArrowDropDownCircle class="d-block icon fill-opacity-1" />
        </button>
      </div>
    </div>

    <div v-show="!collapsed" class="chapper-item__content">
      <div class="chapper-item__learning-item" v-for="(les, i) in lessons" :key="i">
        <div class="chapper-item__learning-icon">
          <IconHeadset class="icon fill-opacity-1" v-if="les.type == 'AUDIO'" />
          <IconVideoCam class="icon fill-opacity-1" v-else-if="les.type == 'VIDEO'" />
          <IconFileCopy class="icon fill-opacity-1" v-else-if="les.type == 'DOCS'" />
          <IconViewList class="icon fill-opacity-1" v-else />
        </div>

        <div class="chapper-item__learning-text">
          <div class="chapper-item__learning-name">
            Bài {{ i + 1 }}: {{les.name}}
            <button class="chapper-item__learning-btn-edit" @click="$emit('edit', les, chapter)">
              <IconEdit class="d-block icon fill-opacity-1" />
            </button>
          </div>
          <div class="chapper-item__learning-info">
            <span class="chapper-item__learning-info-item" v-tooltip="les.file_name || ''">
              <!-- v-for="(doc, index) in les.docs"
              :key="index"-->
              Tên file:
              <span>
                <a :href="les.url" target="_blank">{{ les.file_name ? les.file_name : 'Văn bản' }}</a>
              </span>
            </span>
        ...
        <div class="chapper-item__learning-right">
          <n-link to="/elearning/manager/exams" v-if="!!les.exercises">{{ les.exercises }} Bài tập</n-link>
          <n-link :to="createExerciseUrl" v-else>Thêm bài tập</n-link>
        </div>
      </div>
    </div>
  </div>
