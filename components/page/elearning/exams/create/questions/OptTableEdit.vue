...
export default {
  ...
  data() {
    return {
      ...
      testId: this.$route.query ? this.$route.query.test_id : "",
  ...
  methods: {
    ...
    updateQuestion(){
      ...
      if(this.dataQuestion.method == "RANDOM"){
        ...
        this.$router.push(
          `/elearning/manager/semester/create/question?category=${this.category}&type=${this.type}&parent_id=${this.testId}&question_id=${this.dataQuestion.id}&id=${this.examId}`
        );
      }
    },
