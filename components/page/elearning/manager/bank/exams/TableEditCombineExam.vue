<template>
  <div class="edit-combine-exam">
    <span class="type-exam heading-5 font-weight-normal text-primary">
      I. Trắc nghiệm ({{ getObjBankExamEdit.choice_point }} điểm)
    </span>
      ...
      <table>
        ...
        <draggable tag="tbody">
          <tr v-for="(item, index) in fillterDataExamChoise" :key="index">
            ...
            <td class="nowrap">{{ item.type | exType2TxtTypeQues }}</td>
            ...
            >
              ...
              >
                <div 
                  class="table-exam-tooltip-title"
                >
                  <p class="dont-break-out" v-html="item.select && item.select.content"></p>
                </div>
                <template slot="popover" class="tooltip-detail">
                  <div class="table-exam-tooltip-content" v-html="item.select && item.select.content"></div>
                </template>
            ...
            >
              ...
              >
                <div 
                  class="table-exam-tooltip-title"
                >
                  <p class="dont-break-out" v-html="item.manual && item.manual.content && item.manual.content.replace('{}', index + 1) || '-'"></p>
                </div>
                <template slot="popover" class="tooltip-detail">
                  <div
                   class="table-exam-tooltip-content"
...
export default {
  ...
  computed: {
    ...
    pointMediumChoice(){
      console.log('pointMediumChoice')
      const checkNumberRandom = this.fillterDataExamChoise.reduce((sum, item) => sum + (item.random && item.random.random - 1 || 0), 0);
      const tmp = this.filterPointChoice/(this.fillterDataExamChoise.length+checkNumberRandom);
      return tmp
    },
