      ...
      <div>
        ...
        <div class="box" v-if="typeQuestion == 'SINGLE_CHOICE'">
          <div class="item">
            ...
            <div class="radios">
              <div v-for="(item, index) in get(questions, 'answers', [])"
                :key="index">
              <app-radio
                name="answer"
                @change="answer = item.index"
        ...
        <div class="box" v-if="typeQuestion == 'MULTIPLE_CHOICE'">
          <div class="item">
            ...
            <div class="radios">
              <div v-for="(item, index) in get(questions, 'answers', [])" :key="index">
                <app-checkbox
                  :disabled="true"
                  :checked="item.selected"
                >
              ...
              <div class="col-6">
                <div class="answer-content right">
                  <div class="answer-tooltip">
                    ...
                    <!-- <div class="answer-text">
                      <div v-html="item.matching.content"></div>
                    </div> -->
                  </div>
                </div>
              </div>
        ...
        <div class="box" v-if="typeQuestion == 'FILL_IN_BLANK'">
          <div class="item">
            ...
            <div class="content" v-html="contentQuestion"></div>
          </div>
          <div class="mt-3" v-if="get(questions, 'manual.note', false)">
            <i>{{ questions.manual.note }}</i>
          </div>
...
export default {
  ...
  computed: {
    contentQuestion () {
      let data = this.stringToArray(get({...this.questions}, 'manual.content', ''));
      let content = '';
      if (data.text.length > 0) {
        for (let i = 0; i < data.text.length; i++) {
          let input = data.answers[i] ? "<input type='text'/>" : '';
          content += data.text[i] + input;
        }
      }
      return content;
    },
