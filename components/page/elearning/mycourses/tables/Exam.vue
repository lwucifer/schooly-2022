  ...
  <div>
    ...
    >
      ...
      <template v-slot:cell(status)="{row}">
        <td>
          <span class="nowrap" :class="statusCls(row)" v-if="get(row, 'status', '') == SUBMISSION_RESULTS.NONE">
...
export default {
  ...
  methods: {
    ...
    hasSubmission(item) {
      if ((get(item, "status", '') == SUBMISSION_RESULTS.PASSED)) {
        return true;
      } else if (get(item, 'remain_works', 0) <= 0) {
        return true
      }
      return false;
    },
