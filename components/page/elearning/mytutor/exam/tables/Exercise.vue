...
<script>
  import DropdownExMenu from "~/components/page/elearning/mytutor/exam/DropdownExMenu";
  import { get } from "lodash"
  import IconArrow from "~/assets/svg/v2-icons/arrow_forward_ios_24px.svg?inline"
  import RateStatus from "~/components/page/elearning/mytutor/exam/RateStatus"
  ...
  export default {
    ...
    methods: {
      ...
      goToParticipants(item) {
        this.$router.push(`/elearning/mytutor/exams/${item.id}/participants`);
      },
      gotoPubish(item) {
      ...
      editItem(item) {
        this.$router.push(`/elearning/mytutor/exams/${item.id}/edit-exercise`);
      },
      deleteItem(item) {
