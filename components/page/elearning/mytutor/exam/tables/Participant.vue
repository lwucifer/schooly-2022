      ...
      <template v-slot:cell(action)="{row}">
        <td>
          <n-link
            ...
            title="Chi tiết"
            :to="`/elearning/mytutor/exams/${$route.params.id}/results?user_id=${row.id}&elearning_name=${getParamQuery('elearning_name')}&item_name=${getParamQuery('item_name')}&student_name=${get(row, 'name', '')}&student_id=${row.student_id ? row.student_id: ''}`">
