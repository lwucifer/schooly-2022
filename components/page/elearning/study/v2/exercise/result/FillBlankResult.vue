  ...
  computed: {
    ...
    userAnswer() {
      ...
      [...this.result.userAnswer.submissions]
        ...
        .map(item => {
          question = question.replace("[]", `[${item.answer}]`);
        });
    ...
    correctAnswer() {
      let question = get(this, "result.manual.content", "");
      const numOfPosition = question.split("[]").length;
      for (var pos = 1; pos < numOfPosition; pos++) {
        const contentForPos = this.correctItems
          .filter(f => f.position == pos)
          .reduce((acc, cur) => `${acc}|${cur.content}`, "")
          .slice(1);
        question = question.replace("[]", `[${contentForPos}]`);
      }
      // console.log("[userAnswer] fill", question);
      return question;
    },

    correctItems() {
      const short_answers = this.result.short_answers || [];
      return short_answers.filter(item => item.correct == true);
    },
