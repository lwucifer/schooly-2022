<template>
  <div class="modal-result-answer">
    ...
    <div class="px-4 pt-3" v-if="questionResult != 1">
      <div>Đáp án đúng:</div>
      <fragment v-for="(item, index) in correctItems" :key="index">
        <!-- <span style="min-width: 9rem;">Đáp án đúng: {{ item.indexName }}.</span> -->
        <div class="d-flex">
          <div class="mr-2"><span class="d-block w-20">{{item.indexName}} -</span></div>
          <div
            class="d-flex align-items-center dont-break-out"
            v-html="item.content"
...
export default {
  ...
  computed: {
    ...
    answerItems() {
      console.log("[answerItems] matching", this.result.userAnswer.submissions);
    ...
    correctItems() {
      console.log("[correctItems] matching");
      const corrects = this.result.short_answers
        .filter(item => item.name == "answer")
        .map(m => {
          const matchAns = this.result.short_answers
            .filter(ft => ft.name == "matching")
            .find(f => f.id == m.match_answer_id);
          if (!matchAns) return {};
          return {
            ...m,
            indexName: QUESTION_ANSWER_INDEX_TEXT[m.index],
            content: matchAns.content
          };
        });
      console.log("[correctItems]", corrects);
      return corrects;
    },
