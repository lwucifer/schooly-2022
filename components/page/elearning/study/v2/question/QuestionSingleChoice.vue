<template>
  <div class="elearning-question-type">
    <div class="eqt__content-question">
      <div>
        <span class="">{{ currentExerciseQuestion.index }}</span>
      </div>
  ...
  watch: {
    answer(_newVal, _oldVal) {
      const answers = [{
        ...
        // answer: null, // nội dung đáp án tự nhập (với câu hỏi điền từ)
        // position: null, // vị trí trong nội dung đáp án (với câu hỏi điền từ)
        // matching_answer_id: null // đáp án nối ( với câu hỏi nối )
      }];
    ...
    questionChange: {
      ...
      handler(_newVal, _oldVal) {
        console.log("[questionChange] watch Single_Choice", _newVal);
        this.setAnswered();
      }
    }
