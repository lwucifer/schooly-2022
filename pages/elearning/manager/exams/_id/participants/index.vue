...
import DownloadSubmissionButton from "~/components/page/elearning/manager/exam/DownloadSubmissionButton";
  ...
  methods: {
    ...
    downloadSubmission() {
      try {
        ...
        const fileContent = this.submissionFile;
        const url = window.URL.createObjectURL(new Blob([fileContent], {
          type: 'application/octet-stream'
        }));
        console.log('url: ', url);
        const link = document.createElement("a");
        const fileName = `submission__${this.exercise.data.name}.zip`;
        link.href = url;
        link.setAttribute("download", fileName); //or any other extension
        document.body.appendChild(link);
        link.click();
      } catch (error) {
        console.log('[export submission]', error);
      } finally {
