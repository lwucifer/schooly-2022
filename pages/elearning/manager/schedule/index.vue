          ...
          <div class="mycourses__content">
            ...
            <ul class="timetable-list" v-if="checkDataTable">
              ...
              >
                ...
                <span class="timetable-list-item__title-schedule">
                  <v-popover
                    offset="10"
                    trigger="hover"
                    placement="top"
                  ...
                  >
                    <span>
                      <n-link
                        :to="item.link"
                        :class="item.ended ? 'text-warning' : ''"
                      >{{item.content}}</n-link>
                    </span>
                    <template slot="popover" class="tooltip-detail">
                      <span class="timetable-list-item__title">{{item.content}}</span>
                    </template>
                  </v-popover>
                  <n-link
                    :to="item.link"
                ...
                <span class="timetable-list-item__subtitle-schedule">
                  <!-- {{item.content}} -->
                  {{item.room ? item.room : "--"}}
                </span>
...
const STORE_NAME_SCHEDULE = "elearning/schedule";
...
export default {
  ...
  async fetch({ params, query, store, route }) {
    ...
    await Promise.all([
      store.dispatch(
        `${STORE_NAME_SCHEDULE}/${actionTypes.SCHEDULE.GET_SCHEDULE_LIST}`,
        {
          params: data,
        }
      ),
  ...
  computed: {
    ...mapState(STORE_NAME_SCHEDULE, ["scheduleList"]),

    filterTimeTableList() {
      const data =
        this.scheduleList &&
        this.scheduleList
          .filter((i) => i.type == constants.TYPE_TIME_TABLE.OLCLASS)
          .map((item) => {
            const streaming =
              Date.parse(fullDateTimeSlash(item.start_time)) <=
                moment().valueOf() &&
              moment().valueOf() < Date.parse(fullDateTimeSlash(item.end_time));
            const scheduled =
              Date.parse(fullDateTimeSlash(item.start_time)) >
              moment().valueOf();
            const finished =
              Date.parse(fullDateTimeSlash(item.end_time)) <=
              moment().valueOf();
            console.log(
              "streaming",
              streaming,
            ...
            return {
              ...
              link: streaming
                ? `/elearning/manager/online-class?tab=streaming`
                : scheduled
                ? `/elearning/manager/online-class?tab=scheduled`
                : finished
                ? `/elearning/manager/online-class?tab=finished`
                : `/elearning/manager/online-class`,
              ended:
                item.end_time &&
                Date.parse(fullDateTimeSlash(item.end_time)) <=
                  moment().valueOf(),
    ...
    checkDataTable() {
      return this.scheduleList && this.scheduleList.length > 0 ? true : false;
    },
  ...
  methods: {
    getDateTime_HH_mm,
    ...mapMutations(STORE_NAME_SCHEDULE, ["setStateSchedule"]),
    ...mapActions(STORE_NAME_SCHEDULE, ["getScheduleList"]),
    changeDate(date) {
    ...
    changeDateInWeek(i) {
      ...
      this.setStateSchedule(this.calendar);
      ...
      this.getScheduleList({ params: this.queryFromTo });
