        ...
        <template v-else>
          <div class="row saf-row">
            <div v-for="(friend) in friendsList" :key="friend.id" class="col-md-6 saf-col">
              <AccountFriendItem
                type="birthday"
                :birthday="friend.profile ? friend.profile.birthday : ''"
          ...
          <div v-if="!friendsList.length" class="text-center">Danh sách sinh nhật trống.</div>
          ...
          <client-only>
            ...
            >
              ...
              <template slot="no-results">Danh sách sinh nhật trống.</template>
            </infinite-loading>
          </client-only>
        </template>
      </template>
...
export default {
  ...
  async fetch() {
    if (this.$route.params.uuid !== this.uuidUser) return;
    ...
    async getFriends(page = 0) {
      try {
        const result = await new FriendsService(this.$axios)[ACTION_BASE.LIST]({
          params: {
            fetch_type: FRIENDS_FETCH_TYPE.BIRTHDAY_COMING,
