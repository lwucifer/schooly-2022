  ...
  <AccountPhotoWrapper @add-post-success="handleAddPostSuccess">
    <div v-if="$fetchState.pending" class="row social-photo-row">
      <div class="col-6 col-md-3 social-photo-col" v-for="i in 8" :key="i">
        <VueContentLoading viewBox="0 0 100 100">
    ...
    <template v-else>
      <div class="row social-photo-row">
        <div
          class="col-12 col-sm-6 social-photo-col"
          v-for="(attachment, index) in attachmentsList"
          :key="attachment.id"
        >
          <a
            :href="`/social/post/${attachment.post_id}`"
            class="social-photo-item social-photo-item--video"
      ...
      <div v-if="!attachmentsList.length" class="text-center">Danh sách video trống.</div>
      ...
      <client-only>
        ...
        >
          ...
          <template slot="no-results">Danh sách video trống.</template>
        </infinite-loading>
      </client-only>
    </template>
  ...
  computed: {
    ...
    postDetailAttachment() {
      return {
        ...
        src: this.modalDetailAttachment.url,
      };
    },
  },
    ...
    async handleAddPostSuccess(dataResponse) {
      ...
      this.loadingCount =
        attachments && attachments.length
          ? attachments.filter((item) => item.type === ATTACHMENT_TYPES.VIDEO)
              .length
          : 0;
      ...
      try {
        ...
        if (typeof window.history.pushState != "undefined") {
          window.history.pushState(
            { theater: true },
            "",
            `${window.location.origin}/social/post/${attachment.post_id}?video_id=${attachment.id}`
          );
        } else {
          this.$router.push(
            `social/post/${attachment.post_id}?video_id=${attachment.id}`
          );
          return;
        }
        ...
        const getPost = await this.$store.dispatch(
          ...
          {
            postId: attachment.depend_on_parent
              ? attachment.post_id
              : attachment.id,
            parentPostId: attachment.depend_on_parent
              ? null
              : attachment.post_id,
          }
        );
      ...
      try {
        ...
        const doEdit = async () =>
          await this.$store.dispatch(
            ...
            {
              childPath: isParentPost
                ? post.id
                : `${post.main_post_id || ""}/children/${post.id}`,
              model,
            }
          );
