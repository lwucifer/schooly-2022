<template>
  <main class="page-timeline">
    <div class="bg-white shadow-1">
      <h1 class="heading-4 pa-4">Dòng thời gian</h1>
    ...
    >
      <template v-for="post in feeds">
        <Post
          v-if="post.allow_access || false"
          :key="post.id"
          :post="post"
    ...
    >
      <PostDetail
        ...
        :is-own="modalDetailParentPost && modalDetailParentPost.author ? modalDetailParentPost.author.id === uuidUser : false"
...
export default {
  ...
  computed: {
    ...
    modalDetailParentPost() {
      return (
        this.$store.getters[`social/newfeed/post`](
    ...
    countVisiblePosts() {
      return this.feeds.reduce((sum, post) => {
  ...
  async fetch() {
    const result = await this.$store.dispatch(
      `social/newfeed/${actionTypes.NEWFEED.GET_FEEDS}`
    );
  ...
  methods: {
    ...
    async handleClickImage({ id, url, object }, index, total, post, parentSharePost) {
      ...
      if (post.attachments.length === 1) {
        // If post is a shared post -> get post
        if (parentSharePost) {
    ...
    handleCloseModal() {
      if (typeof window.history.pushState != "undefined") {
        window.history.pushState({}, "", `${window.location.origin}/social`);
      }
    ...
    async handleSubmitEditPost(data, clearData, setLoading) {
      try {
        ...
        const doEdit = async () =>
          await this.$store.dispatch(
            `social/newfeed/${actionTypes.NEWFEED.EDIT_POST}`,
        ...
        const result = await Promise.all([
          ...
          privacy &&
            ...
            this.$store.dispatch(
              `social/newfeed/${actionTypes.NEWFEED.EDIT_POST_PRIVACY}`,
              {
                postId,
                privacy,
              }
    ...
    async feedInfiniteHandler($state) {
      ...
      const getData = await this.$store.dispatch(
        `social/newfeed/${actionTypes.NEWFEED.GET_FEEDS_INFINITE}`,
        {
    ...
    async deletePost(id) {
      ...
      const doDelete = await this.$store.dispatch(
        `social/newfeed/${actionTypes.NEWFEED.DELETE_POST}`,
        id
      );
    ...
    async likePost(isLiked, post, emoji, cb) {
      ...
      await this.$store.dispatch(
        `social/newfeed/${
          isLiked
            ? actionTypes.NEWFEED.DELETE_LIKE_POST
            : actionTypes.NEWFEED.LIKE_POST
        }`,
        {
    ...
    async getComment(
      ...
      const getComment = await this.$store.dispatch(
        `social/newfeed/${actionTypes.NEWFEED.GET_COMMENT}`,
        {
          id: post.id,
    ...
    async postComment(post, { content, listTags, image, link }, clearEditor) {
      ...
      const doPostComment = await this.$store.dispatch(
        `social/newfeed/${actionTypes.NEWFEED.ADD_COMMENT}`,
        {
          postId: post.id,
          model,
        }
    ...
    async editComment({ postId, commentId, content }, cancelEdit) {
      ...
      const doEdit = await this.$store.dispatch(
        `social/newfeed/${actionTypes.NEWFEED.EDIT_COMMENT}`,
        {
          postId,
          commentId,
          model,
    ...
    async deleteComment({ id, postId }) {
      const doDelete = await this.$store.dispatch(
        `social/newfeed/${actionTypes.NEWFEED.DELETE_COMMENT}`,
        { id, postId }
      );
    ...
    async likeComment(isLiked, postId, parentCommentId, commentId, emoji, cb) {
      ...
      await this.$store.dispatch(
        `social/newfeed/${
          isLiked
            ? actionTypes.NEWFEED.DELETE_LIKE_COMMENT
            : actionTypes.NEWFEED.LIKE_COMMENT
        }`,
        {
          postId,
          commentId,
    ...
    async getChildComment(
      ...
      const doGet = await this.$store.dispatch(
        `social/newfeed/${actionTypes.NEWFEED.GET_CHILD_COMMENT}`,
        {
          postId,
          commentId,
          fromId,
    ...
    async postChildComment({ content, postId, commentId }, clearEditor) {
      ...
      const doAction = await this.$store.dispatch(
        `social/newfeed/${actionTypes.NEWFEED.ADD_CHILD_COMMENT}`,
        {
          postId,
          commentId,
          model,
    ...
    async editChildComment(
      ...
      const doEdit = await this.$store.dispatch(
        `social/newfeed/${actionTypes.NEWFEED.EDIT_CHILD_COMMENT}`,
        {
          postId,
          commentId,
          parentCommentId,
    ...
    async deleteChildComment({ id, postId, parentCommentId }) {
      const doDelete = await this.$store.dispatch(
        `social/newfeed/${actionTypes.NEWFEED.DELETE_CHILD_COMMENT}`,
    ...
    async likeChildComment(
      ...
      const doLike = await this.$store.dispatch(
        `social/newfeed/${
          isLiked
            ? actionTypes.NEWFEED.DELETE_LIKE_CHILD_COMMENT
            : actionTypes.NEWFEED.LIKE_CHILD_COMMENT
        }`,
        {
          model,
          postId,
    ...
    async editTagsOfPost({ post, isParentPost, tags }) {
      return isParentPost
        ? await this.$store.dispatch(
            `social/newfeed/${actionTypes.NEWFEED.EDIT_POST_TAGS}`,
            {
              postId: post.id,
              tags,
            }
