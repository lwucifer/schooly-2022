<template>
  <main class="page-social-post-detail">
    <VclFacebook v-if="$fetchState.pending" class="bg-white w-100 pa-4" />
    ...
    <template v-else>
      <a
        ref="postCommentScroll"
        v-scroll-to="'#post__comment-editor'"
        style="display: none"
      ></a>
      <Post
        :post="post"
        ...
        @get-comment-before="getCommentBefore"
      ...
      >
        ...
        >
          <template
            slot="media-content"
            slot-scope="{ attachments, sharedPost }"
          >
            <PostImage
          ...
          >
            <template v-for="comment in commentTree || []">
              ...
              >
                <template
                  slot="child-comment"
                  slot-scope="{ commentTree, handleLevel2Reply }"
                >
                  <transition-group
                    enter-active-class="animated faster fadeIn"
                    leave-active-class="animated faster fadeOut"
      ...
      >
        <PostDetail
          ...
          :parent-post="post.type === POST_TYPES.SHARE ? post.payload : post"
          ...
          :is-own="
            modalDetailParentPost && modalDetailParentPost.author
              ? modalDetailParentPost.author.id === uuidUser
              : false
          "
      ...
      >
        <template slot="share-content">
          <app-content-box
            v-if="
              shareData.type === POST_TYPES.LINK &&
                typeof shareData.link === 'object'
            "
            tag="a"
            target="_blank"
            class="my-0"
          ...
          <PostShareContent v-else :post="shareData">
            <template slot="media-content">
              <PostImage
                v-if="
                  shareData.type === POST_TYPES.MEDIA ||
                    shareData.attachments ||
                    shareData.attachment
                "
                class="mt-0 mb-4"
                :images="
                  shareData.attachments
                    ? shareData.attachments.map((item) => ({
                        id: item.id,
                        thumb: get(item, 'url.medium', null),
                        object: item.type,
                        url: item.url,
...
export default {
  ...
  data() {
    return {
      ...
      cmtId: this.$route.query ? this.$route.query.commentId : "",
      cmtCheckScroll: "",
    };
  },

  computed: {
    ...mapState("social/post", ["post"]),
    ...
    modalDetailParentPost() {
      if (this.post.type === POST_TYPES.SHARE) {
  ...
  async fetch() {
    try {
      const result = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.GET_POST}`,
  ...
  mounted() {
    this.cmtCheckScroll = this.$route.query ? this.$route.query.commentId : "";
    if (process.browser) {
      const photoId = this.$route.query.photo_id;
      photoId && this.showModalOnInital(photoId);
  ...
  watch: {
    ...
    "$route.query.commentId": function(_newVal, _oldVal) {
      this.scrollTo();
    },

    cmtCheckScroll(_newVal) {
      _newVal && this.scrollTo();
    },
  },
  ...
  methods: {
    ...
    scrollTo() {
      setTimeout(() => {
        this.$refs.postCommentScroll && this.$refs.postCommentScroll.click();
      }, 1000);
    },

    showModalOnInital(photoId) {
      const { attachments = [] } = this.post;
      const photoObj =
        attachments && attachments.find((item) => item.id === photoId);
      const photoIndex =
        attachments && attachments.findIndex((item) => item.id === photoId);
      if (!photoObj) return;

      this.handleClickImage(
        { id: photoId, url: photoObj.url },
        photoIndex,
        attachments.length,
        this.post
      );
    ...
    handleCloseModal() {
      if (typeof window.history.pushState != "undefined") {
        window.history.pushState(
          {},
          "",
          `${window.location.origin}/social/post/${this.post.id}`
        );
      }
    ...
    async handleSubmitEditPost(data, clearData, setLoading) {
      try {
        ...
        const doEdit = async () =>
          await this.$store.dispatch(
            `social/post/${actionTypes.SOCIAL_POST.EDIT_POST}`,
        ...
        const result = await Promise.all([
          ...
          privacy &&
            ...
            this.$store.dispatch(
              `social/post/${actionTypes.SOCIAL_POST.EDIT_POST_PRIVACY}`,
              {
                postId,
                privacy,
              }
    ...
    async sharePost(data, cb) {
      ...
      const doShare = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.ADD_POST}`,
        dataWithModel
      );
    ...
    async deletePost(id) {
      ...
      const doDelete = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.DELETE_POST}`,
        id
      );
    ...
    async likePost(isLiked, post, emoji, cb) {
      ...
      await this.$store.dispatch(
        `social/post/${
          isLiked
            ? actionTypes.SOCIAL_POST.DELETE_LIKE_POST
            : actionTypes.SOCIAL_POST.LIKE_POST
        }`,
        {
    ...
    async getComment(
      ...
      const getComment = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.GET_COMMENT}`,
        {
          id: post.id,
          from_comment_id: fromId,
          fetch_size: PAGE_SIZE.DEFAULT,
          includes: this.cmtId ? 'yes' : null
        }
      );
      ...
      if (this.cmtId) {
        await this.$store.dispatch(
          `social/post/${actionTypes.SOCIAL_POST.CHECK_SHOW_MORE_TOP}`,
          {
            id: postId,
            from_comment_id: data.length > 0 ? data[0].id : "",
            fetch_size: PAGE_SIZE.DEFAULT,
    ...
    /**
     * To GET comment before of post
     */
    async getCommentBefore(
      postId,
      fromId,
      setIsFetchingComment,
      setIsCommentFetched,
      setShowViewMoreCommentBefore
    ) {
      setIsFetchingComment && setIsFetchingComment(true);

      const getComment = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.GET_COMMENT_BEFORE}`,
        {
          id: postId,
          from_comment_id: fromId,
          fetch_size: PAGE_SIZE.DEFAULT,
      ...
      // if (setShowViewMoreCommentBefore && data.length < PAGE_SIZE.DEFAULT) {
      //   setShowViewMoreCommentBefore(false);
      // } else {
      //   setShowViewMoreCommentBefore(true);
      // }
      ...
      if (data.length == 10) {
        ...
        this.$store.commit(
          `social/post/${mutationTypes.SOCIAL_POST.SET_SHOW_MORE_TOP}`,
          0
        );
      }
    },
    ...
    async postComment(post, { content, listTags, image, link }, clearEditor) {
      ...
      const doPostComment = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.ADD_COMMENT}`,
        {
          postId: post.id,
          model,
        }
    ...
    async editComment({ postId, commentId, content }, cancelEdit) {
      ...
      const doEdit = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.EDIT_COMMENT}`,
        {
          postId,
          commentId,
          model,
    ...
    async deleteComment({ id, postId }) {
      const doDelete = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.DELETE_COMMENT}`,
        { id, postId }
      );
    ...
    async likeComment(isLiked, postId, parentCommentId, commentId, emoji, cb) {
      ...
      await this.$store.dispatch(
        `social/post/${
          isLiked
            ? actionTypes.SOCIAL_POST.DELETE_LIKE_COMMENT
            : actionTypes.SOCIAL_POST.LIKE_COMMENT
        }`,
        {
          postId,
          commentId,
    ...
    async getChildComment(
      ...
      const doGet = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.GET_CHILD_COMMENT}`,
        {
          postId,
          commentId,
          fromId,
    ...
    async postChildComment({ content, postId, commentId }, clearEditor) {
      ...
      const doAction = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.ADD_CHILD_COMMENT}`,
        {
          postId,
          commentId,
          model,
    ...
    async editChildComment(
      ...
      const doEdit = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.EDIT_CHILD_COMMENT}`,
        {
          postId,
          commentId,
          parentCommentId,
    ...
    async deleteChildComment({ id, postId, parentCommentId }) {
      const doDelete = await this.$store.dispatch(
        `social/post/${actionTypes.SOCIAL_POST.DELETE_CHILD_COMMENT}`,
    ...
    async likeChildComment(
      ...
      const doLike = await this.$store.dispatch(
        `social/post/${
          isLiked
            ? actionTypes.SOCIAL_POST.DELETE_LIKE_CHILD_COMMENT
            : actionTypes.SOCIAL_POST.LIKE_CHILD_COMMENT
        }`,
        {
          model,
          postId,
    ...
    async editTagsOfPost({ post, isParentPost, tags }) {
      return isParentPost
        ? await this.$store.dispatch(
            `social/post/${actionTypes.SOCIAL_POST.EDIT_POST_TAGS}`,
            {
              postId: post.id,
              tags,
            }
